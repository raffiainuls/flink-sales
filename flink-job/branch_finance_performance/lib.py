...
def create_branch_finance_performance():
    create_sql_branch_finance_performance = """
    CREATE TABLE branch_finance_performance (
       ...
       'connector' = 'upsert-kafka',
        'topic' = 'branch_finance_performance',
        'properties.bootstrap.servers' = 'host.docker.internal:9093',
        'key.format' = 'json',
        'value.format' = 'json'
    )
    """
    return create_sql_branch_finance_performance

def insert_into_branch_finance_performance(table_env):
    logger.info("🔁 Inserting data into sink table...")
    table_env.execute_sql("""
    INSERT INTO branch_finance_performance
       -- unit_sold  
        with uss as (
        select 
        tb.name,
